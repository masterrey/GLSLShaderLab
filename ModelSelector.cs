...
namespace GLSLShaderLab
{
    public class ModelSelector
    {
        public class ModelInfo
        {
            ...
            public override string ToString() => $"{Name} ({Path.GetFileName(FilePath)})";
        }
        ...
        public ModelInfo? TryGetModelByFileName(string fileName)
        {
        ...
        {
            var preferred = new[] { "cube.glb", "CuboUVFull.glb", "sphere.glb", "monkey.glb" };
            foreach (var fileName in preferred)
            {
                var found = TryGetModelByFileName(fileName);
                if (found != null) return found;
            }

            return _availableModels.FirstOrDefault();
        }

        public ModelInfo? SelectModel(bool showAdvancedPrompt = true)
        {
            ...
            {
                Console.WriteLine("Dica: para foco didático, use um modelo simples (ex.: cube/sphere).\n");
            }
            ...
            if (input == "r")
            {
                ...
                _availableModels = ScanForModels();
                return SelectModel(showAdvancedPrompt);
            }

            if (int.TryParse(input, out int selection) && selection >= 1 && selection <= _availableModels.Count)
            {
